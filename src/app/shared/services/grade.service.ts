...
export class GradeService {
   ...
   */
  async addGrade(
    gradeData: Omit<Grade, 'id' | 'tenantId' | 'createdAt' | 'updatedAt'>,
  ): Promise<string | null> {
    const tenantId = await this.auth.getTenantIdForCurrentUser();
    ...
    try {
      ...
      if (gradeData.submittedDate !== undefined)
        cleanData['submittedDate'] = gradeData.submittedDate;
   ...
   */
  async getStudentGradeSummaries(
    classId: string,
